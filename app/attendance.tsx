...
export default function Attendance() {
  ...
  const [timeField, setTimeField] = useState<'in' | 'out' | null>(null);

  const timeDate = (value: string, fallback: string) => {
    const [hour, minute] = (validTime(value) ? value : fallback).split(':').map(Number);
    const date = new Date(); date.setHours(hour, minute, 0, 0); return date;
  };
  const displayTime = (value: string) => {
    if (!validTime(value)) return 'Pilih waktu';
    const [hour, minute] = value.split(':').map(Number);
    return new Date(2000, 0, 1, hour, minute).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
  };
  const selectTime = (_event: DateTimePickerEvent, value?: Date) => {
    const field = timeField; setTimeField(null);
    if (!field || !value) return;
    const formatted = `${String(value.getHours()).padStart(2, '0')}:${String(value.getMinutes()).padStart(2, '0')}`;
    if (field === 'in') setCheckIn(formatted); else setCheckOut(formatted);
  };
  ...
  return <KeyboardAvoidingView style={s.page} behavior={Platform.OS==='ios'?'padding':'height'}><ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    ...
    {isAbsence ? <View style={s.infoCard}><Text style={s.infoTitle}>Pengajuan Tidak Hadir</Text><Text>{attendance?.status}</Text>{!!attendance?.note && <Text style={s.muted}>{attendance.note}</Text>}</View> : isToday ? <>
      ...
      <Text style={s.step}>Pengisian manual</Text><Text style={s.muted}>Khusus untuk melengkapi kekurangan absen pada tanggal yang sudah lewat.</Text>
      <View style={s.row}><View style={s.half}><Text style={s.label}>Jam masuk</Text><Pressable style={s.timePicker} onPress={() => setTimeField('in')}><Text style={s.timePickerText}>{displayTime(checkIn)}</Text></Pressable></View><View style={s.half}><Text style={s.label}>Jam pulang</Text><Pressable style={s.timePicker} onPress={() => setTimeField('out')}><Text style={s.timePickerText}>{displayTime(checkOut)}</Text></Pressable></View></View>
    ...
    {timeField && <DateTimePicker value={timeDate(timeField === 'in' ? checkIn : checkOut, timeField === 'in' ? '06:30' : '14:30')} mode="time" display="clock" is24Hour={false} onChange={selectTime} />}

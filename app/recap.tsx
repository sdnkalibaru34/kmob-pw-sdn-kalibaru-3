...
export default function Recap(){
 ...
 const load=async()=>{setBusy(true);setMessage('');try{
   ...
   setItems(calendar.map(day=>{const a=map.get(day.work_date);if(!a)return{date:day.work_date,state:'Belum absen',detail:day.description??'Hari kerja'};
     if(a.status==='Hadir'&&!a.check_in)return{date:day.work_date,state:'Belum jam masuk',detail:a.status};
     if(a.status==='Hadir'&&!a.check_out)return{date:day.work_date,state:'Belum jam pulang',detail:a.status};
     return{date:day.work_date,state:'Lengkap',detail:a.status};}));
 ...
 {items.map(x=><Pressable key={x.date} style={s.card} onPress={()=>router.push({pathname:'/attendance',params:{date:x.date}})}><View><Text style={s.date}>{x.date}</Text><Text style={s.detail}>{x.detail}</Text></View><View style={s.right}><Text style={[s.state,x.state==='Lengkap'&&s.ok]}>{x.state}</Text><Text style={s.open}>Buka</Text></View></Pressable>)}
 </ScrollView>
}
const s=StyleSheet.create({page:{flex:1,backgroundColor:'#f7faf8'},content:{padding:24,paddingTop:64,paddingBottom:48,gap:12},title:{fontSize:28,fontWeight:'800'},reload:{alignSelf:'flex-start',paddingVertical:10,paddingHorizontal:14,borderRadius:10,backgroundColor:'#e3f2e8'},reloadText:{color:'#18794e',fontWeight:'700'},card:{backgroundColor:'#fff',padding:15,borderRadius:14,borderWidth:1,borderColor:'#e0e9e3',flexDirection:'row',justifyContent:'space-between',alignItems:'center'},right:{alignItems:'flex-end',gap:3},date:{fontWeight:'800'},detail:{color:'#647168',marginTop:3},state:{color:'#b42318',fontWeight:'800'},ok:{color:'#18794e'},open:{fontSize:12,color:'#526158'}});

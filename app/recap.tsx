...
export default function Recap(){
 ...
 {items.map(x=><View key={x.date} style={s.card}><View><Text style={s.date}>{x.date}</Text><Text style={s.detail}>{x.detail}</Text></View><Text style={[s.state,x.state==='Lengkap'&&s.ok]}>{x.state}</Text></View>)}
 </ScrollView>
}
const s=StyleSheet.create({page:{flex:1,backgroundColor:'#f7faf8'},content:{padding:24,paddingTop:64,paddingBottom:48,gap:12},title:{fontSize:28,fontWeight:'800'},reload:{alignSelf:'flex-start',paddingVertical:10,paddingHorizontal:14,borderRadius:10,backgroundColor:'#e3f2e8'},reloadText:{color:'#18794e',fontWeight:'700'},card:{backgroundColor:'#fff',padding:15,borderRadius:14,borderWidth:1,borderColor:'#e0e9e3',flexDirection:'row',justifyContent:'space-between',alignItems:'center'},date:{fontWeight:'800'},detail:{color:'#647168',marginTop:3},state:{color:'#b42318',fontWeight:'800'},ok:{color:'#18794e'}});

...
export default function Recap(){
 ...
 <Pressable style={s.print} onPress={()=>router.push('/personal-monthly-recap')}><Text style={s.printText}>Cetak / Download Rekap Bulanan</Text></Pressable>
 ...
 {items.map(x=>{const noInfo=x.state==='Tanpa Keterangan';const incomplete=x.state.startsWith('Belum absen');const complete=x.state==='Absensi lengkap';return <Pressable key={x.date} style={s.card} onPress={()=>router.push({pathname:'/attendance',params:{date:x.date}})}><View style={s.left}><Text style={s.date}>{x.date}</Text><Text style={s.detail}>{x.detail}</Text></View><View style={s.right}><Text style={[s.state,noInfo&&s.noInfo,incomplete&&s.incomplete,complete&&s.complete]}>{x.state}</Text><Text style={s.open}>Buka</Text></View></Pressable>})}
 </ScrollView>
}
const s=StyleSheet.create({page:{flex:1,backgroundColor:'#f7faf8'},content:{padding:24,paddingTop:64,paddingBottom:48,gap:12},title:{fontSize:28,fontWeight:'800'},print:{backgroundColor:'#225a91',padding:14,borderRadius:11,alignItems:'center'},printText:{color:'#fff',fontWeight:'800'},reload:{alignSelf:'flex-start',paddingVertical:10,paddingHorizontal:14,borderRadius:10,backgroundColor:'#e3f2e8'},reloadText:{color:'#18794e',fontWeight:'700'},card:{backgroundColor:'#fff',padding:15,borderRadius:14,borderWidth:1,borderColor:'#e0e9e3',flexDirection:'row',justifyContent:'space-between',alignItems:'center',gap:12},left:{flex:1},right:{alignItems:'flex-end',gap:5,maxWidth:155},date:{fontWeight:'800'},detail:{color:'#647168',marginTop:3,lineHeight:18},state:{color:'#7a3e00',fontWeight:'800',textAlign:'right',paddingHorizontal:9,paddingVertical:5,borderRadius:8,overflow:'hidden'},noInfo:{backgroundColor:'#c62828',color:'#fff'},incomplete:{backgroundColor:'#f5c842',color:'#111'},complete:{backgroundColor:'#18794e',color:'#fff'},open:{fontSize:12,color:'#526158'}});

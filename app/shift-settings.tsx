...
export default function ShiftSettings(){
  ...
  const save=async()=>{setBusy(true);setMessage('');try{await updateOwnWorkPreference(shift,workPattern);const result=await scheduleAttendanceReminders(shift,workPattern,true);const reminderMessage=result.ok?' Pengingat absensi telah dijadwalkan ulang.':result.reason==='permission-denied'?' Aktifkan izin notifikasi agar pengingat dapat tampil.':' Pengingat belum dapat dijadwalkan. Periksa izin alarm dan pengaturan baterai aplikasi.';setMessage(`${workPattern==='Opsi 1'?'5 hari kerja':'6 hari kerja'} · shift ${shift} berhasil disimpan.${reminderMessage}`) }catch{setMessage('Pengaturan jam kerja belum dapat diubah. Coba kembali.')}finally{setBusy(false)}};

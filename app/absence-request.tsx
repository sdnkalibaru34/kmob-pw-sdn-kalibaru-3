...
import { ownWorkPreference, submitAbsenceRequest } from '@/lib/data';
import { scheduleText } from '@/lib/schedule';
import type { AbsenceStatus, ShiftLabel, WorkPattern } from '@/lib/types';
...
export default function AbsenceRequest() {
  ...
  const selectDate=(_event:DateTimePickerEvent,value?:Date)=>{const field=dateField;setDateField(null);if(!field||!value)return;const selected=`${value.getFullYear()}-${String(value.getMonth()+1).padStart(2,'0')}-${String(value.getDate()).padStart(2,'0')}`;if(field==='start'){setStartDate(selected);if(endDate<selected)setEndDate(selected)}else setEndDate(selected)};
  useEffect(()=>{void ownWorkPreference().then(value=>{setShift(value.shift);setWorkPattern(value.workPattern)}).catch(()=>setMessage('Jam kerja belum dapat dimuat.'))},[]);
  const save=async()=>{
    ...
    if(!scheduleText(startDate,shift,workPattern)&&startDate===endDate)return setMessage('Tanggal tersebut bukan hari kerja.');
    if(!note.trim())return setMessage('Keterangan pengajuan wajib diisi.');
    setBusy(true);setMessage('');
    try{const total=await submitAbsenceRequest({startDate,endDate,status,note,shiftLabel:shift,workPattern});setMessage(`Pengajuan ${status} berhasil disimpan untuk ${total} hari kerja.`)}

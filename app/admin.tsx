...
export default function Admin(){
 const [allowed,setAllowed]=useState<boolean|null>(null);const [month,setMonth]=useState(new Date().toLocaleDateString('en-CA',{timeZone:'Asia/Jakarta'}).slice(0,7));const [employees,setEmployees]=useState<Employee[]>([]);const [attendance,setAttendance]=useState<Attendance[]>([]);const [reports,setReports]=useState<DailyReport[]>([]);const [workdays,setWorkdays]=useState<string[]>([]);const [message,setMessage]=useState('');const [busy,setBusy]=useState(false);
 const sync=async()=>{setBusy(true);setMessage('');try{const result=await syncGoogleSheets(month);setMessage(`Spreadsheet berhasil disinkronkan untuk ${result.month}.`)}catch(error){setMessage(error instanceof Error?error.message:'Sinkronisasi gagal.')}finally{setBusy(false)}};
 const load=async()=>{setBusy(true);setMessage('');try{const{data:user}=await supabase.auth.getUser();const isAdmin=user.user?.app_metadata?.role==='admin';setAllowed(isAdmin);if(!isAdmin)return;
   ...
   ]);if(e.error||a.error||r.error||c.error)throw new Error();setEmployees((e.data??[]) as Employee[]);setAttendance((a.data??[]) as Attendance[]);setReports((r.data??[]) as DailyReport[]);const today=new Date().toLocaleDateString('en-CA',{timeZone:'Asia/Jakarta'});setWorkdays((c.data??[]).map(x=>x.work_date).filter(x=>x<today));
 ...
 return <ScrollView style={s.page} contentContainerStyle={s.content}><Text style={s.title}>Dashboard Admin</Text><Text style={s.sub}>Rekap seluruh pegawai · SDN Kalibaru 3</Text>
  <View style={s.controls}><TextInput style={s.input} value={month} onChangeText={setMonth} placeholder="YYYY-MM"/><Pressable style={s.button} disabled={busy} onPress={load}><Text style={s.buttonText}>{busy?'Memuat…':'Tampilkan'}</Text></Pressable><Pressable style={[s.button,s.syncButton]} disabled={busy} onPress={sync}><Text style={s.buttonText}>Sinkronkan Spreadsheet</Text></Pressable></View>{!!message&&<Text>{message}</Text>}
  <Text style={s.heading}>Rekap Bulanan</Text><ScrollView horizontal><View>
   <View style={[s.row,s.header]}>{['Nama','Jabatan','Absen Masuk','Izin','Sakit','Cuti','Dinas Luar','Tanpa Ket.','Hari Kerja'].map(x=><Text key={x} style={[s.cell,s.headText]}>{x}</Text>)}</View>
...
const s=StyleSheet.create({page:{flex:1,backgroundColor:'#f7faf8'},content:{padding:28,paddingTop:50,paddingBottom:60,maxWidth:1200,width:'100%',alignSelf:'center'},center:{flex:1,alignItems:'center',justifyContent:'center',padding:24},denied:{color:'#b42318',fontWeight:'800'},title:{fontSize:30,fontWeight:'900',color:'#18794e'},sub:{marginTop:5,marginBottom:20,color:'#5f6d64'},controls:{flexDirection:'row',flexWrap:'wrap',gap:10,marginBottom:18},input:{borderWidth:1,borderColor:'#cfd8d3',backgroundColor:'#fff',borderRadius:10,padding:12,width:140},button:{backgroundColor:'#18794e',paddingHorizontal:18,paddingVertical:12,justifyContent:'center',borderRadius:10},syncButton:{backgroundColor:'#2563a6'},buttonText:{color:'#fff',fontWeight:'800'},heading:{fontSize:21,fontWeight:'800',marginTop:20,marginBottom:10},row:{flexDirection:'row',backgroundColor:'#fff',borderBottomWidth:1,borderBottomColor:'#e2e9e4'},header:{backgroundColor:'#e4ece7'},cell:{width:115,padding:11,textAlign:'center'},name:{width:230,textAlign:'left',fontWeight:'600'},headText:{fontWeight:'800'},card:{backgroundColor:'#fff',padding:15,borderRadius:12,borderWidth:1,borderColor:'#e0e9e3',marginBottom:8,gap:5},cardTitle:{fontWeight:'800',color:'#18794e'},muted:{color:'#647168'}});
